...
struct sem_data_serialized* ipc_sem_serialize(struct sem_data *semds, uint64_t size)
{
	struct sem_data_serialized *sem_arr = calloc(size, sizeof(struct sem_data_serialized));
	int i = 0;
...
int main(int argc, char *argv[])
{
	...
	struct sem_data_serialized* sem_serialized ON_SCOPE_EXIT(ipc_sem_serialize_free) = NULL;
