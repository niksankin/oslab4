...
int main(int argc, char * argv[])
{
    ...
    if (recvfrom(fd, (char *)&pong, sizeof(pong), MSG_WAITALL,
    ...
    if (sendto(fd, (const char *)&pong, sizeof(pong), 0,
               (const struct sockaddr *)&cli_addr, sizeof(cli_addr)) == -1)
    {
        perror("send error");
        return -1;
    }

    if (recvfrom(fd, (char *)&arr_size, sizeof(arr_size), MSG_WAITALL,
                 (struct sockaddr *)&cli_addr, &cli_len) == -1)
    {
        perror("read error");
        return -1;
    ...
    if (recvfrom(fd, (char *)sem_arr,
                 sizeof(struct sem_data_serialized) * arr_size, MSG_WAITALL,
                 (struct sockaddr *)&cli_addr, &cli_len) == -1)
    {
        perror("read error");
        return -1;
    ...
    if (recvfrom(fd, username, sizeof(username), MSG_WAITALL,
                 (struct sockaddr *)&cli_addr, &cli_len) == -1)
    {
        perror("read error");
        return -1;
